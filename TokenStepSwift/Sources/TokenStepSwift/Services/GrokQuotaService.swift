...
enum GrokQuotaService {
    static var authURL: URL = grokHomeURL().appendingPathComponent("auth.json")

    static func grokHomeURL() -> URL {
    ...
    static func read() throws -> ProviderQuota {
        ...
        if !windows.isEmpty {
            ...
            var metrics = productMetrics(from: lastConfig)
            metrics.grokWeeklyUsed = weekly
    ...
    static func productMetrics(from config: [String: Any]) -> QuotaMetrics {
        let products = productWindows(from: config, kind: .weekly, reset: nil)
        func used(_ title: String) -> Double? {
            products.first(where: { $0.title == title })?.usedPercent
        }
        return QuotaMetrics(
            grokBuildUsed: used("Grok Build"),
            grokImagineUsed: used("Imagine"),
            grokChatUsed: used("Chat"),
            grokVoiceUsed: used("Voice"),
            grokApiUsed: used("API"),
            grokBotUsed: used("Grok Bot")
        )
    }
    ...
    static func windows(from payload: Any) -> [QuotaWindow] {
        ...
        let period = (config["currentPeriod"] as? [String: Any])
            ...
            ?? (object["currentPeriod"] as? [String: Any])
            ?? (object["current_period"] as? [String: Any])
        let periodEnd = period?["end"]
            ?? config["billingPeriodEnd"]
            ?? config["billing_period_end"]
            ?? object["billingPeriodEnd"]
            ?? object["billing_period_end"]
        let creditUsed = QuotaJSON.number(
            config["creditUsagePercent"]
                ?? config["credit_usage_percent"]
                ?? object["creditUsagePercent"]
        ...
        if creditUsed != nil || period != nil {
            ...
            let kind: QuotaWindowKind = type.contains("MONTH") ? .monthlyCredits : .weekly
            let reset = date(from: periodEnd)
    ...
    private static func clientVersion() -> String {
        let url = grokHomeURL().appendingPathComponent("version.json")
    ...
    private static func date(from value: Any?) -> Date? {
        ...
        if let date = ISO8601DateFormatter.grokNoFraction.date(from: text) {
            return date
        }
        // Trim >3 fractional digits that Foundation's ISO8601 parser rejects.
        if let dot = text.firstIndex(of: "."),
           let zone = text.lastIndex(of: "+") ?? text.lastIndex(of: "Z"),
           zone > dot {
            ...
            let normalized = String(text[..<text.index(after: dot)]) + trimmed + String(text[zone...])
            if let date = ISO8601DateFormatter.grok.date(from: normalized) {
                return date
            }
            return ISO8601DateFormatter.grokNoFraction.date(from: normalized)
        }
        return nil
    }
